...
import { Upload, FileText, X, CheckCircle2, Circle, Database, Loader2, BookOpen, Search, Info, ChevronDown, ChevronUp } from 'lucide-react';
          ...
          <div>
            <div className="flex items-center gap-2">
              <div className={`p-1.5 rounded-lg transition-colors ${isCollapsed ? 'bg-gray-100' : 'bg-blue-50'}`}>
                <Database className={`w-4 h-4 ${isCollapsed ? 'text-gray-400' : 'text-[#0077B5]'}`} />
              </div>
              <h3 className={`text-sm font-bold uppercase tracking-wide transition-colors ${isCollapsed ? 'text-gray-400' : 'text-gray-800'}`}>
                Knowledge Base {isCollapsed && documents.length > 0 && <span className="ml-1 text-[10px] text-[#0077B5]">({documents.length})</span>}
              </h3>
            </div>
            {!isCollapsed && <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1 animate-in fade-in duration-300">Add context for the AI</p>}
          </div>
          ...
          <div className="flex items-center gap-2">
            {!isCollapsed && (
              <button
                ...
                className="bg-[#0077B5] text-white px-3 py-1.5 rounded-lg text-[10px] hover:bg-[#005a8a] transition-all flex items-center gap-2 font-bold shadow-sm animate-in zoom-in-95 duration-200"
              >
                <Upload className="w-3 h-3" /> Upload File
              </button>
            )}
            <div className="text-gray-400 group-hover:text-[#0077B5] transition-all ml-1">
